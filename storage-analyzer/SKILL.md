...
- **macOS**：扫 home、library、caches、containers、group_containers、app_support、applications、downloads、dev_caches，并额外扫 system_library、system_developer、core_simulator、private_var，用 `du` 算大小。额外系统级分组是为了拆开 macOS「系统数据」里的真实大头，例如 Xcode 模拟器 runtime、dyld 缓存、系统级旧插件和 `/private/var` 日志/缓存。
...
macOS 分析时不要只信 `home` / `library` 分组。必须同时看：
- `core_simulator`：解释 Xcode/iOS/watchOS/tvOS/xrOS runtime 和 dyld 缓存，常被 macOS 归进「系统数据」。
- `system_developer` / `system_library`：解释 CommandLineTools、系统级插件、旧软件残留、字体、打印机、Adobe/Alipay 等全局组件。
- `private_var`：解释诊断日志、uuidtext、var/folders 临时缓存等系统数据；多数只做说明，不建议手删。
...
3. **追清 macOS「系统数据」大头**：若用户提到系统数据很大，必须核对 `/Library/Developer/CoreSimulator`、`/Library/Developer`、`/Library`、`/private/var`，并用 `xcrun simctl runtime list` 与 `diskutil apfs list` 区分 Xcode 可识别 runtime、APFS 挂载卷实际占用、dyld 缓存和普通目录。不要把 `/Library/Developer/CoreSimulator/Volumes` 当普通文件夹建议 `rm`。
4. **三级分类 = 清理决策清单，不是全盘点。** 只把"存在'要不要动它'这个决策"的项放进三灯；日常在用的正常应用、操作系统本身、海量零碎小文件没有清理决策，不进三灯，它们落在磁盘条的蓝色"系统及其他"里。判定标准：
...
macOS 常见特殊项：
- **Xcode 模拟器 runtime**：`/Library/Developer/CoreSimulator/Volumes/*` 和 `/Library/Developer/CoreSimulator/Caches/dyld` 通常归 🟡。说明应引导用户用 Xcode Settings > Platforms 或 `xcrun simctl runtime list` 判断/删除不用的 runtime；不要给直接删除按钮，不要建议迁到 NAS。`~/Library/Developer/Xcode/DerivedData` 才通常归 🟢。
- **应用 VM/运行时镜像**：如 `Claude-3p/vm_bundles/rootfs.img`、`sessiondata.img`、`.zst`，归 🟡。只把明确的 `Cache` / `GPUCache` 子目录归 🟢。
- **备份暂存包**：如 `CodexBackupToolkit/spool/*.tar.gz`，即使是压缩包也先归 🟡。检查 manifest，说明备份范围；建议保留最新一份、确认 NAS/外部副本后再清旧包。
- **卸载残留**：先核对 `/Applications` 是否仍有主程序。用户目录残留（如 `~/Library/PPS`、`~/Library/Application Support/QvodPlayer`）可给橙灯 `trash_paths`；`/Library` 下系统级插件（Flash、Adobe AIR、Alipay、QuickLook 等）需要管理员权限，报告只给说明/命令，不给网页删除按钮。
- **文稿分类**：macOS「文稿」不等于 `~/Documents`；微信/邮件/浏览器 profile 里的 PPT/PDF/Office 文件也可能被归入文稿。用 `find "$HOME" -xdev -type f` 按扩展名抽样大文件辅助判断。
